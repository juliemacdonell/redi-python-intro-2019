...
all_people = teachers.union(students)
...
median = statistics.median(final_grades.values())
print(f'The median for the final grades is: {median}')

try:
    mode = statistics.mode(final_grades.values())
except statistics.StatisticsError:
    print(f'There is no unique mode for the final grades!')
else:
    print(f'The mode for the final grades is: {mode}')


# https://docs.python.org/3/library/random.html
import random

favorite_student = random.choice(students).capitalize()
print(f'My favorite student is: {favorite_student}')
